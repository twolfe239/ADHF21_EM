...
uint8_t ssd1306_Init(void) {
	...
	if (HAL_I2C_IsDeviceReady(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 5, 1000)
			!= HAL_OK) {
...
void ssd1306_FillRect(int16_t xMove, int16_t yMove, int16_t width,
		int16_t height) {
...
void ssd1306_DrawFillTriangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
void ssd1306_DrawCircleQuads(int16_t x0, int16_t y0, int16_t radius,
		uint8_t quads) {
	int16_t x = 0, y = radius;
	int16_t dp = 1 - radius;
	while (x < y) {
...
void ssd1306_DrawProgressBar(uint16_t x, uint16_t y, uint16_t width,
...
#else
	HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1, data, size,
			100);
#endif
}
